...
```
> **補足:** `NEXT_PUBLIC_SUPABASE_PROXY_URL` を指定すると、ブラウザからのリクエストは Next.js 経由で Supabase に転送され、CORS を回避できます。Vercel のプレビュー URL でも動作させるため、`/api/supabase` のような **相対パス** で設定することを推奨します（ローカルでは `http://localhost:3000/api/supabase` でも可）。未設定の場合は従来どおり Supabase の `Project URL` に直接接続します。
...
4. **環境変数の設定**
   ...
   - ブラウザアクセスを常に許可したい場合は、`NEXT_PUBLIC_SUPABASE_PROXY_URL` に `/api/supabase`（ローカル開発なら `http://localhost:3000/api/supabase`）を設定し、常に同一オリジン経由で Supabase REST を呼び出すようにします
